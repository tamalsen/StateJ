...
public class SymbolicConstraintsGeneral {
	  ...
	  Object getExpression(IntegerExpression eRef) {
			...
			case MINUS:
				if (e_leftRef instanceof IntegerConstant && e_rightRef instanceof IntegerConstant){
					//System.out.println("SymbolicConstraintGeneral.java:89 left: "+ e_leftRef+ " right:" + e_rightRef);
					throw new RuntimeException("## Error: this is not a symbolic expression"); //
				}
				else if (e_leftRef instanceof IntegerConstant)
	...
	public boolean isSatisfiable(PathCondition pc) {
		if (pc == null || pc.count() == 0) {
			 //Commented by Tamal 13/1/2012
			//didn't like SymbolicInstructionFactory to be used here
...
//				}
		...
		//System.out.println("SymbolicConstraintGeneral.java:980 ::solve() has been called from here also.");
		result = pb.solve();

		/*if (SymbolicInstructionFactory.debugMode){
			System.out.println("numeric PC: " + pc + " -> " + result);
			System.out.println("SymbolicConstraintGeneral.java:985 ::Hi");
		}*/
	...
	public boolean isSatisfiable(Constraint cons) {
		if(cons==null){
			return true;
		}
		else{
		...
		String[] dp = new String[1];
		dp[0]="choco";
			pb = new ProblemChoco();//new ProblemCoral();//new ProblemIAsolver();//new ProblemChoco2();
			
		symRealVar = new HashMap<SymbolicReal,Object>();
		symIntegerVar = new HashMap<SymbolicInteger,Object>();
		//result = null;

		Constraint cRef = cons;
		...
		if(result == null) {
			System.out.println("## Warning: timed out/ don't know (returned Constraint not-satisfiable) "+cons);
			return false;
		}
		if (result == Boolean.TRUE) {
			return true;
